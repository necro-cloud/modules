  ...
  manifest = {
    ...
    "spec" = {
      "dnsNames" = [
        ...
        "grafana",
        "grafana.${kubernetes_namespace.namespace.metadata[0].name}.svc.cluster.local",
        "*.grafana.${kubernetes_namespace.namespace.metadata[0].name}.svc.cluster.local",
      ]
      "subject" = {
        "organizations"       = [var.organization_name]
...
// Cloudflare Issuer for Observability Ingress Service
...
// Certificate to be used for Observability Ingress
