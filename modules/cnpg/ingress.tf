...
resource "kubernetes_ingress_v1" "api_ingress" {
  metadata {
    ...
    annotations = {
      ...
      "nginx.ingress.kubernetes.io/proxy-ssl-secret" : "${kubernetes_namespace.namespace.metadata[0].name}/${kubernetes_manifest.pgadmin_internal_certificate.manifest.spec.secretName}"
      ...
      "nginx.ingress.kubernetes.io/permanent-redirect" : "/pgadmin4"
    }
  }
    ...
    rule {
      ...
      http {
        path {
          ...
          backend {
            service {
              name = kubernetes_service.pgadmin4.metadata[0].name
              port {
                name = "https"
              }
            }
